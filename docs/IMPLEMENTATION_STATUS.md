...
- 最小クレートとして `app-core`、`render`、`ui-shell`、`plugin-api`、`storage`、`plugin-host`、`panel-sdk`、`apps/desktop` がある
...
- `desktop` は `apps/desktop/ui/` 配下の `.altp-panel` を再帰ロードし、組み込み標準パネルをファイルベースで読み込める
- `apps/desktop/ui/panels/` 配下に標準パネルごとの独立フォルダがある
...
- `crates/panel-dsl` / `crates/panel-schema` / `crates/panel-sdk` を追加し、フェーズ6の最小土台を着手した
- `panel-dsl` は `.altp-panel` の parser / validator / normalized IR を持ち、handler binding を抽出できる
- `ui-shell` は `apps/desktop/ui/` から `.altp-panel` をロードし、static panel をホスト描画へ接続できる
- `plugin-api::HostAction` に handler 呼び出し用の最小 variant を追加し、将来の Wasm runtime 接続点を確保した
- `crates/plugin-host` を追加し、`wasmtime` ベースの最小 Wasm panel runtime を導入した
- `ui-shell` は DSL panel の local state と host snapshot を持ち、Wasm handler の state patch / command descriptor を反映できる
- `apps/desktop/ui/phase6-sample.altp-panel` と対応 Wasm runtime により、サンプル UI が実際に表示・操作できる
- `apps/desktop/ui/panels/` に標準パネル6種の UI DSL + Rust SDK + Wasm 版を追加した
- 旧 `crates/builtin-plugins` 実装を削除し、標準パネルはファイルベースの DSL/Wasm 版へ統一した
- `ui-shell` は `.altp-panel` を再帰探索し、独立フォルダ化した標準パネルをまとめてロードできる
- `scripts/build-ui-wasm.ps1` を追加し、clone 直後でも標準パネル Wasm と `phase6-sample.wasm` を再生成できるようにした
- host snapshot に document summary / active layer / jobs / snapshot status を拡張し、フェーズ7移植向けの読み取りデータを増やした

現時点では、「フェーズ3: 保存と再読込」は最小形で実装済みであり、
「フェーズ4: パネル中間表現の確立」は完了、
「フェーズ5: 標準パネルの移植」は最小フォーカス/クリック/スクロールを含む形で到達したと見てよく、
「フェーズ6: パネル基盤 crate と UI DSL parser」はロードマップ上の完了条件を満たすところまで完遂した。

また、「フェーズ7: 既存ビルトイン panel の UI DSL + Wasm 移植」は標準パネル6種について到達しており、既定 UI ディレクトリ配下の独立フォルダから DSL/Wasm 版を直接ロードする経路へ移行した。
...
- `crates/panel-sdk` を追加し、command descriptor 構築 helper を置いた
...
- `apps/desktop/ui/phase6-sample.altp-panel` と `phase6-sample.wasm` を追加し、保存・読込・ツール切替・詳細展開のサンプル UI を表示できるようにした
...
- `apps/desktop/ui/` に `builtin.app-actions` / `builtin.tool-palette` / `builtin.layers-panel` の `.altp-panel` と対応 Wasm runtime を追加した
...
- `ui-shell` / `desktop` のテストで、DSL 版 `app-actions` の `Command` 発行、DSL 版 `tool-palette` の active 表示、DSL 版 `layers-panel` の host snapshot 表示を検証した

### 到達点

- 少なくとも 3 種類の既存ビルトイン panel を UI DSL + Wasm 版へ差し替えて動かせる
- built-in 専用 ABI を増やさず、既存の command descriptor → `Command` 変換経路を再利用できている
- Rust 実装をフォールバックとして残したまま、既定 UI ディレクトリに置いた DSL/Wasm 版を優先できる

### 現時点の制約

- `color-palette` / `job-progress` / `snapshot-panel` の移植は未着手である
- `color-palette` を移植するには `slider` / `color-preview` の DSL ノード化または同等の UI 記述拡張がまだ必要である
- 式評価は依然として単純な式に限定されており、複合文字列補間は未対応である
