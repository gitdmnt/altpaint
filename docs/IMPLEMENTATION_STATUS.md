...
- 最小クレートとして `app-core`、`render`、`ui-shell`、`plugin-api`、`storage`、`plugin-host`、`panel-sdk`、`panel-dsl`、`panel-schema`、`apps/desktop` がある
...
現在は、「フェーズ8: 外部 Wasm パネルランタイム」の一般化に向けた土台も `plugin-host` クレートによりほぼ整っている状態である。
...
- `panel_handle_event` などの ABI を通じて、UI DSL 上のイベントを Wasm 側へ配送し、結果をホスト側へ書き戻す経路を確立した
