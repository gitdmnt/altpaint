...
| BL-149 | tool-palette のサイズ記憶 (config.size_memory JSON blob) をホスト `EditorSession.per_tool_sizes` へ移管。**B10 で実装完了**: 記憶/復元のメカニズム (キー計算・per-tool マップ・ペン index 計算) をホストへ集約し、パネルは `SessionCommand` の `remember_size` フラグ (`select_tool_remembering` 等) で記憶要否のみ表明する。ドロップダウン/バケツ系は非記憶 (`remember_size: false`) を維持し挙動不変。session 保存形式変更は `#[serde(default)]` で旧 JSON も既定値読込可。rotation 自前計算は該当コード消滅済みで対象なし | god | tool-palette, editor-state, panel-runtime, panel-sdk | BL-142 | B10 |
