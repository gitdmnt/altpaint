...
| 9C-2 | ステータステキスト GPU 化 + L1 背景フレーム廃止 ✅ (9E-4 統合) | 1 セッション  | 9C-1       |
| 9D   | L3 ブラシプレビュー overlay の GPU 化 ✅                   | 1 セッション  | 9B         |
| 9E   | DSL パネル GPU 直描画化 (HtmlPanelEngine 経路統合) ✅       | 5 サブセッション | 9C, 9D     |
| 9F   | 残存 CPU 経路の駆除と `crates/render/` ディレクトリ削除 (**次フェーズ**) | 1 セッション  | 9E         |
...
## 次のフェーズ: Phase 9F — render クレート物理削除

Phase 9E 完了 (2026-04-26) で DSL パネル + ステータスバーの GPU 化が達成され、
`crates/render/` の残存表面は次のみ:
...
- `blit_scaled_rgba_region` / `build_source_axis_runs` / `fill_rgba_block`
  / `scroll_canvas_region` / `SourceAxisRun`
- `PanelHitKind` / `PanelHitRegion` (ui-shell 互換型)

Phase 9F の作業範囲:

1. 上記残存 API の呼び出し元 (主に L1 base_layer の dummy 経路 / L4 ui_panel_layer
   の dummy 経路 / 一部テスト) を撤去
2. `PresentScene::base_layer` / `PresentScene::ui_panel_layer` の **型自体** を削除
3. `PresentScene::html_panel_quads` を `PresentScene::panel_quads` にリネーム
   (status quad と DSL/HTML パネル quad を同一レイヤーで扱う)
4. `PanelHitKind` / `PanelHitRegion` を `ui-shell` または `panel-api` に移管
5. `crates/render/` ディレクトリと workspace member 登録を削除
6. ADR `docs/adr/010-render-crate-removal.md` を作成

完了条件: workspace 内に `use render::` の参照が 0 件、`cargo test --workspace`
通過、`cargo clippy` 警告増加なし。
