...
## Phase 9F 完了記録 (2026-04-29)

`crates/render/` ディレクトリを物理削除し、workspace 内 `use render::` 参照を 0 件に。

実施内容:

1. `RenderFrame` / `RenderContext` を `apps/desktop/src/app/canvas_frame.rs::CanvasFrame` /
   `build_canvas_frame()` に吸収
2. `PanelHitKind` / `PanelHitRegion` を `crates/ui-shell/src/presentation.rs` 内部型に移管
3. `PresentScene::base_layer` (L1) / `ui_panel_layer` (L4) `FrameLayer` を撤去
4. `PresentScene::html_panel_quads` を `panel_quads` にリネーム、レイヤー番号を再付番
5. `PresentTimings` から `base_upload` / `ui_panel_upload` 系フィールド削除
6. `compose_*` / `blit_*` / `fill_rgba_block` / `scroll_canvas_region` /
   `build_source_axis_runs` / `SourceAxisRun` は呼び出し元 0 件で削除のみ
7. `crates/render/` ディレクトリと workspace member 登録を削除
8. ADR `docs/adr/010-render-crate-removal.md` を作成
