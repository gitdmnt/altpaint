...
| 8B | 🔶 部分実装 (2026-04-19) | `GpuBrushDispatch` + WGSL 3本 (`brush_stroke`, `brush_stamp`, `erase_stamp`)。`execute_paint_input` で GPU dispatch を呼び出しているが、**CPU パスと並行稼働**中 |
| 8C | ⬜ 未着手 | GPU テクスチャを表示の正本へ切り替え |
| 8D | ⬜ 未着手 | CPU ペイントパスの廃止 |
| 8E | ⬜ 未着手 | GPU 塗りつぶし + レイヤー合成 |
