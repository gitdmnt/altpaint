//! export feature スライス (provisional)。
//!
//! コマ合成ビットマップの PNG 書き出しを所有する (BL-119 / R31)。
//! B7 で `project-store::export` から移管した。
...
fn write_png(bitmap: &RgbaBitmap, path: &Path) -> Result<(), ExportError> {
    ...
    let mut encoder = png::Encoder::new(writer, bitmap.width as u32, bitmap.height as u32);
